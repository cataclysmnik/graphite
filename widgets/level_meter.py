...
class LevelMeter(QWidget):
    ...
    def paintEvent(self, event):
        ...
        try:
            ...
            if w > h:
                # Horizontal mode
                margin_x = 2 if h < 20 else 5
                clip_width = 4 if w < 100 else 8
                margin_y = 1 if h < 20 else 2
                meter_x_start = margin_x
                meter_width = w - margin_x - clip_width - margin_x
                meter_height = h - (margin_y * 2)
                
                # 3. Draw Clip LED at the right
                clip_rect = QRectF(w - margin_x - clip_width, margin_y, clip_width, meter_height)
                if self.clip_active:
                ...
                num_segments = 12 if w < 100 else 24
                seg_spacing = 1.0 if w < 100 else 1.5
                total_spacing_w = (num_segments - 1) * seg_spacing
                seg_w = max(1.0, (meter_width - total_spacing_w) / num_segments)
                ...
                for i in range(num_segments):
                    ...
                    x_pos = meter_x_start + i * seg_w + i * seg_spacing
                    seg_rect = QRectF(x_pos, margin_y, seg_w, meter_height)
                ...
                if self.peak_db > min_db:
                    ...
                    peak_x = meter_x_start + (norm_peak * meter_width)
                    ...
                    painter.drawLine(int(peak_x), int(margin_y), int(peak_x), int(h - margin_y))
            else:
